...
- **智能后处理**: 规则引擎自动纠正常见中文识别错误（同音纠错 + 拼音匹配）
- **智能标点**: CT-Transformer 语义标点恢复 + 自动优化中文标点
- **语义 Emoji**: 根据说话内容自动添加表情符号
- **音频预处理**: 高通滤波 + 谱减降噪，提升文件转写场景识别精度
- **识别历史**: 自动保存识别记录，支持搜索和统计
- **系统托盘**: 最小化运行，不干扰日常工作
- **剪贴板友好**: 输入后自动恢复原始剪贴板内容
- **文件转写**: 支持音视频文件批量转写，输出 SRT 字幕
...
首次启动会自动下载 Whisper small 模型（约 460MB）。详细安装说明见 [install.md](docs/project/install.md)。
...
- 录音快捷键和触发方式
- 识别模型大小 (tiny/base/small/medium/large)
- 识别语言
- 热词管理（预置词库 + 自定义）
- 后处理规则开关
- 识别精度选项（音频预处理、降噪强度、同音纠错）
- Emoji 开关和密度
- 识别历史记录
- 主题切换 (系统/亮色/暗色)
